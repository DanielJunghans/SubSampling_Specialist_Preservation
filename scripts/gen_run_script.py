...
OUTPUT_DIR = './output/'
SCRIPT_FILENAME = './run_experiments.sh'
...
def get_lexicase_lines(pop_size, num_tests, pass_prob):
    s = ''
...
def get_cohort_lines(pop_size, num_tests, pass_prob):
    s = ''
...
def get_downsampled_lines(pop_size, num_tests, pass_prob):
    s = ''
    for sub_level in subsampling_levels_L:
...
def get_tournament_lines(pop_size, num_tests, pass_prob):
    s = ''
...
def get_roulette_lines(pop_size, num_tests, pass_prob):
    s = ''
    for rep_id in range(1, NUM_REPLICATES + 1):
        s += get_generic_line(
            ...
            for num_tests in num_tests_L:
                ...
                for pass_prob in pass_prob_L:
                    ...
                    if scheme_map['lexicase']:
                        fp.write(get_lexicase_lines(pop_size, num_tests, pass_prob))
                    if scheme_map['cohort']:
                        fp.write(get_cohort_lines(pop_size, num_tests, pass_prob))
                    if scheme_map['downsampled']:
                        fp.write(get_downsampled_lines(pop_size, num_tests, pass_prob))
                    if scheme_map['tournament']:
                        fp.write(get_tournament_lines(pop_size, num_tests, pass_prob))
                    if scheme_map['roulette']:
                        fp.write(get_roulette_lines(pop_size, num_tests, pass_prob))
    # Make file executable by all
    cur_mode = os.stat(SCRIPT_FILENAME).st_mode
    os.chmod(SCRIPT_FILENAME, cur_mode | stat.S_IXUSR | stat.S_IXGRP | stat.S_IXOTH)
